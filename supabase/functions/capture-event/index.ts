...
function getTextExtractionPrompt(defaultTimezone?: string): string {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      const { mode, image, media_type, event, transcript, text, timezone: bodyTimezone, submission_type, pending_event_id, edits } = body;
      ...
      if (mode === "extract-text") {
        ...
        const extractedEvent = await callClaude([
          { type: "text", text: `${getTextExtractionPrompt(bodyTimezone)}\n\nText:\n${text}` },
        ]);
